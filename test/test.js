...
describe('Add user', function(){
    it('Should add a user ', function(){
        ...
        expect(user).to.eql({status : true, msg: 'User created', user: {userName: 'user1', creditLimit: 300}});         
    });
    it('Should not add a user - user already exit', function(){
        var user = payLater.addUser('user1', 'u1@users.com', 300);
        expect(user).to.eql({status : false, msg: 'User already exist'});                  
    });
    it('Should add a user ', function(){
        var user = payLater.addUser('user2', 'u2@users.com', 400);
        expect(user).to.eql({status : true, msg: 'User created', user: {userName: 'user2', creditLimit: 400}});         
    });
    it('Should add a user ', function(){
        var user = payLater.addUser('user3', 'u3@users.com', 500);
        expect(user).to.eql({status : true, msg: 'User created', user: {userName: 'user3', creditLimit: 500}});         
    });
});
...
describe('Add Merchant', function(){
    it('Should add a merchant ', function(){
        ...
        expect(merchant).to.eql({status : true, msg: 'Merchant created', merchant: {merchantName : 'm1', discount: 0.5}});         
    });
    it('Should not add a merchant - merchant already exist', function(){
        var merchant = payLater.addMerchant('m1', 'm1@merchants.com', 0.5);         
        expect(merchant).to.eql({status : false, msg: 'Merchant already'});         
    });
    it('Should add a merchant ', function(){
        var merchant = payLater.addMerchant('m2', 'm2@merchants.com', 1.5);         
        expect(merchant).to.eql({status : true, msg: 'Merchant created', merchant: {merchantName : 'm2', discount: 1.5}});         
    });
    it('Should add a merchant ', function(){
...
describe('Transaction by user to merchant', function(){
    it('Transaction should fail from user2 to m1 with less credit limit', function(){
        ...
        expect(transaction).to.eql({status : false, msg: 'rejected!', reason: 'credit limit'});                  
    });
    it('Transaction from user1 to m2 with credit limit should be succesfull', function(){
        ...
        expect(transaction).to.eql({status : true, msg: 'success!'});                  
    });
    ...
    it('Transaction should fail from user1 to m1 where user doesnot exist', function(){
        ...
        expect(transaction).to.eql({status : false, msg: 'rejected!', reason: 'user doesnot exist'});                  
    });
    it('Transaction should fail from user1 to m15 where mercent m15 doesnot exist', function(){
        ...
        expect(transaction).to.eql({status : false, msg: 'rejected!', reason: 'mercent doesnot exist'});                  
    });
});
...
describe('Payback by user', function(){
    it('Should return the remaining due amount', function(){
        ...
        expect(result).to.eql({user: 'user3', dues : 100});
    });
});
...
describe('Report total dues', function(){
    it('Should return all users with dues along with due amount', function(){
        ...
        expect(usersAndDues).to.eql({dues:[{user: 'user1', dues: 300}, {user: 'user3', dues: 100}], total : 400});         
    });
});
...
describe('Update merchant discount ', function(){
    it('Should update discount by merchant', function(){
        ...
        expect(updatedDiscount).to.eql({merchant: 'm3', discount : 2.0});
    });
});